...
import time
import shutil
from src.themes import ThemeManager  # Absolute import
from src.tools import ToolManager  # Absolute import
from src.queue_manager import QueueManager  # Absolute import
from src.config import THEME, ICONS, STYLE  # Absolute import
import gi
import psutil
import queue
...
"""
    ...
    def clear_queue(self, button=None):
        ...
        if not self.queue_manager.clear_queue():
            self.update_terminal_output("Kuyruk temizlenemedi")
            return
            
        # Butonları aktif et
        self.start_queue_button.set_sensitive(True)
        self.clear_queue_button.set_sensitive(True)
...
"""
    ...
    def add_to_queue_clicked(self, button=None):
        """Seçili aracı kuyruğa ekle"""
        if not hasattr(self, 'selected_tool_id'):
            return
            
        tool_info = self.terminal_tools[self.selected_tool_id]
        if self.queue_manager.add_package(tool_info['package'], tool_info['name']):
            # Butonları aktif et
            self.start_queue_button.set_sensitive(True)
            self.clear_queue_button.set_sensitive(True)
            self.queue_button.set_sensitive(False)
    ...
    def clear_queue(self, button=None):
        ...
        if self.queue_manager.clear_queue():
            # Kuyruk başarıyla temizlendi
            self.start_queue_button.set_sensitive(False)  # Başlat butonu devre dışı
            self.clear_queue_button.set_sensitive(False)  # Temizle butonu devre dışı
            self.update_terminal_output("Sıra temizlendi")
        else:
            # Kuyruk temizlenemedi
            self.update_terminal_output("Sıra temizlenemedi - muhtemelen işlemler devam ediyor")
