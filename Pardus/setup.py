...
def post_install():
    ...
    # Ana program izinlerini ayarla
    main_script = os.path.join(SHARE_DIR, "main.py")
    shutil.copy2("main.py", main_script)
    subprocess.run(["chmod", "+x", main_script])
    
    # ToolsGet başlatıcı betiğini kopyala
    starter_path = "/usr/local/bin/toolsget-starter"
    shutil.copy2("ToolsGet", starter_path)
    subprocess.run(["chmod", "+x", starter_path])
    
    # Sembolik bağlantı oluştur
    subprocess.run(["ln", "-sf", starter_path, "/usr/bin/toolsget"])
    
    # Desktop dosyasını sistem ve kullanıcı masaüstüne kopyala
    desktop_file = "tr.org.pardus.toolsget.desktop"
    
    # Sistem uygulamaları klasörüne kopyala
    system_apps = "/usr/share/applications/"
    shutil.copy2(desktop_file, system_apps)
    
    # Kullanıcı masaüstüne kopyala
    user = os.environ.get('SUDO_USER', os.environ.get('USER'))
    user_desktop = f"/home/{user}/Masaüstü"
    if os.path.exists(user_desktop):
        desktop_path = os.path.join(user_desktop, desktop_file)
        shutil.copy2(desktop_file, desktop_path)
        # Doğru kullanıcı sahipliğini ayarla
        subprocess.run(["chown", f"{user}:{user}", desktop_path])
        subprocess.run(["chmod", "+x", desktop_path])
    
    print("\nKurulum tamamlandı!\n")
    
    # Programı otomatik başlat
    if user != "root":
        try:
...
def download_program():
    ...
    try:
        ...
        repos = [
            "https://github.com/e-Ey-cyber/Terminal-Tools-Grafik-Aray-z-/tree/main/Pardus",
            "https://github.com/e-Ey-cyber/Terminal-Tools-Grafik-Aray-z-/tree/main/Pardus",
            "https://github.com/e-Ey-cyber/Terminal-Tools-Grafik-Aray-z-/tree/main/Pardus"
        ]
